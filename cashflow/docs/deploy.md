...
- доступ к ВМ по SSH;
- доступ суперпользователя к базе (см. шаг 0 — узнаем, как он называется);
- доступ в личный кабинет Т-Бизнеса (чтобы выпустить токен);
- доступ в консоль Yandex Cloud.

---

## Шаг 0. Где живёт база

PostgreSQL бывает установлен прямо на машину, а бывает запущен в контейнере Docker.
От этого зависят все команды ниже. Проверьте:

```bash
docker ps
```

Если в списке есть контейнер с образом `postgres` (например `teashop-postgres`) —
**база в Docker**, и дальше вы идёте по вариантам «в Docker». Если команда ничего
не вывела или Docker не установлен — база на самой машине, вариант «на хосте».

Для базы в Docker узнайте имя администратора и имя базы (пароль команда не покажет):

```bash
docker inspect teashop-postgres --format '{{range .Config.Env}}{{println .}}{{end}}' | grep -v PASSWORD
```

Запишите значения `POSTGRES_USER` и `POSTGRES_DB` — они понадобятся дальше.
Если этих строк нет, значит используются значения по умолчанию: пользователь и база
называются `postgres`.

Дальше в инструкции: `АДМИН` — это `POSTGRES_USER`, `ИМЯ_БАЗЫ` — `POSTGRES_DB`,
`КОНТЕЙНЕР` — имя контейнера из `docker ps`.

## Шаг 1. Безопасность: сначала копии

Пока ничего не меняли — сделайте так, чтобы любую ошибку можно было откатить.

1. **Снимки диска ВМ.** В консоли Yandex Cloud: Compute Cloud → Диски → ваш диск →
   Расписания снимков. Поставьте ежедневные снимки и хранение хотя бы 7 дней.
...
2. **Копия базы.**
   ...
   ```bash
   docker exec КОНТЕЙНЕР pg_dump -U АДМИН ИМЯ_БАЗЫ | gzip > ~/before-cashflow-$(date +%F).sql.gz
   ls -lh ~/before-cashflow-*.sql.gz
   ```
   ...
   Проверьте, что файл не пустой — он должен весить хотя бы несколько килобайт.
...
Если база в Docker (обратите внимание на `-i` и на `<` в конце — файл подаётся
внутрь контейнера):
...
docker exec -i КОНТЕЙНЕР psql -U АДМИН -d ИМЯ_БАЗЫ \
  ...
  < migrations/001_schema_and_roles.sql
```
...
```bash
docker exec КОНТЕЙНЕР psql -U АДМИН -d ИМЯ_БАЗЫ -c "\dn"     # для Docker
sudo -u postgres psql -d ИМЯ_БАЗЫ -c "\dn"                   # для базы на хосте
```

В списке должна появиться схема `finance`, а все прежние схемы — остаться на месте.
...
(дата открытия счёта), `PGDATABASE` (это `ИМЯ_БАЗЫ`), `PGPASSWORD` (пароль роли
`finance_sync`, который вы придумали на шаге 6). Токен пока оставьте пустым —
впишем на следующем шаге.

`PGHOST=127.0.0.1` и `PGPORT=5432` подходят в обоих случаях: и когда база на хосте,
и когда она в Docker с проброшенным портом. Проверить подключение можно так:

```bash
PGPASSWORD='ПАРОЛЬ_ДЛЯ_СИНХРОНИЗАЦИИ' psql -h 127.0.0.1 -U finance_sync -d ИМЯ_БАЗЫ -c "select current_user"
```

Если `psql` на машине не установлен (частый случай, когда база в Docker), поставьте
клиент — сам сервер он не трогает:
...
```bash
PGPASSWORD='ПАРОЛЬ_ДЛЯ_СИНХРОНИЗАЦИИ' psql -h 127.0.0.1 -U finance_sync -d ИМЯ_БАЗЫ -c \
