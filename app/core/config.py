...
class Settings(BaseSettings):
    ...
    #
    # Строкой, а не числом: сюда естественно вписать и id, и короткое имя
    # страницы (`nikita_kvitko`), и ссылку на неё. Числовое поле на коротком
    # имени роняло всё приложение при старте — настройки не читались, и
    # контейнер не поднимался вовсе. Имя переводит в id `vk_client`.
    admin_vk_id: str = ""
    ...
    # ближайший рабочий день». Дни недели все — магазин отвечает и в
    # выходные, поэтому отдельной настройки под нерабочие дни нет.
    manager_work_hours_start: int = 10
    manager_work_hours_end: int = 23
    # Тихие часы: ночью бот сам клиенту не пишет, напоминания ждут утра.
    # Начало совпадает с концом рабочего дня менеджера: пока он отвечает,
    # бот тоже может написать, а дальше молчат оба.
    ...
    # Закрывающий чек (зачёт предоплаты) при вручении посылки: коды
    # маркировки каждой пачки, полный расчёт. По умолчанию выключен —
    # включать после проверки на тестовом магазине ЮKassa.
    settlement_receipt_enabled: bool = False
    # В каком виде код маркировки идёт в `mark_code_info.gs_1m`. Документация
    # ЮKassa двоякая: в OpenAPI — «код в формате GS1.M», а пример в статье
    # про чек зачёта — base64 от кода с разделителями GS внутри; модуль ЮKassa
    # для 1С-Битрикс умеет оба варианта и по умолчанию шлёт base64. Держим
    # настройкой, пока поддержка не ответит: `base64` или `raw`.
    yookassa_mark_code_encoding: str = "base64"
    # Часовая зона магазина для чека с маркированным товаром (тег 1011):
    # 2 — Москва, UTC+3. Обязательна, когда в чеке есть коды маркировки.
    yookassa_receipt_timezone: int = 2
    # Через сколько часов «pending» у закрывающего чека — повод сказать
    # менеджеру. ЮKassa просит обращаться к ним через трое суток; мы
    # говорим раньше, чтобы было время разобраться.
    settlement_receipt_pending_alert_hours: int = 24
